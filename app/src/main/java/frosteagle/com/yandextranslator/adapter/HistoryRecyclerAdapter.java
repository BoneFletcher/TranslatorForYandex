...
public class HistoryRecyclerAdapter  extends RecyclerView.Adapter<HistoryRecyclerAdapter.ViewHolder> implements RealmChangeListener {
        ...
        public HistoryRecyclerAdapter(List<FavoriteHistory> histories, Context context, Realm realm) {
          //  mRealm = realm;
...
//            mHistoriesCopy.addChangeListener(this);
        ...
        public void onBindViewHolder(final ViewHolder holder, final int position) {
            ...
            holder.imgHistory.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    mRealm.beginTransaction();
                    FavoriteHistory historyModelRealm = new FavoriteHistory();
                    historyModelRealm.setFirstWord(mHistories.get(position).getFirstWord());
                    historyModelRealm.setSecondWord(mHistories.get(position).getSecondWord());
                    historyModelRealm.setLocale(mHistories.get(position).getLocale());
                    historyModelRealm.setFavorite(!mHistories.get(position).isFavorite());
                    mRealm.copyToRealmOrUpdate(historyModelRealm);
                    mRealm.commitTransaction();
                    if (historyModelRealm.isFavorite()) {
                        holder.imgHistory.setImageResource(R.drawable.ic_turned_in_yellow);
                    } else {
                        holder.imgHistory.setImageResource(R.drawable.ic_turned_in_grey);
                    }
